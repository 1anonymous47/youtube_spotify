function downloader()
{
    ...
    .then(data=>{
        if(data['data']=="success")
        {
            ...
            sound.src      = 'static/audios/'+data['audiosrc'];
            ...
            button.onclick =()=> playlistadder(data['audiosrc'],data['audiotittle']);
...
function playlistadder(audiosrc,audiotittle)
{
    const username = document.getElementById('username').value;
    fetch('/playlistadder',{
        method: "POST",
        ...
        body: JSON.stringify({ "username":username,"audiosrc":audiosrc,"audiotittle":audiotittle }),
    })
    .then(res=>res.json())
    .then(data=>{
        if(data['data']=="success")
